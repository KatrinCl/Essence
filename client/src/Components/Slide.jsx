...
const Slide = ({ slide }) => {
  return (
    <div className="bg-white rounded-xl p-5 flex flex-col h-[320px] md:h-[350px] w-full">
      ...
      <div className="mt-4 flex flex-col gap-3 flex-1">
        {slide.details.map((detail, index) => (
          <div key={index} className="flex items-center gap-2 w-full text-xs md:text-sm">
            ...
            {/* описание */}
            <p className="text-black max-w-[60%]">
              {detail.description}
            </p>
            ...
            {/* частота */}
            <p className="text-gray-500 whitespace-nowrap text-right">
              {detail.frequency}
            </p>
